...
export default class ValidationContract {
	...
	isCNPJ(value: CNPJProps, message: string): void {
		if (!value) {
			this.errors.push({ message });
			return;
		}
		console.log(Object.keys(value).length);
		const cleanedCNPJ = normalizerCNPJ(value); // Remove caracteres não numéricos
		...
		const cnpjWithoutDigits: string = cleanedCNPJ.slice(0, 12);
