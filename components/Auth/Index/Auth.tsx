...
export default function AuthScreen() {
		...
		switch (field) {
			...
			case "confirmPassword":
				if (value !== formData.password) {
					contract.confirmKey(value, formData.password, "As senhas não coincidem");
				}
				break;
		}
	...
	const isValidUser = async (): Promise<boolean> => {
		...
		if (!isLogin) {
            contract.confirmKey(formData.password, formData.confirmPassword, "As senhas não coincidem");
			// Validação do nome
			if (userType === "driver") {
				contract.hasMinLen(
					formData.name,
